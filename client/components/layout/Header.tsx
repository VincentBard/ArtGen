...
export function Header() {
  ...
  const handleSearch = (e: React.FormEvent) => {
    ...
    if (searchQuery.trim()) {
      // Navigate to gallery with search query - the gallery page will handle the actual search
      navigate(`/gallery?search=${encodeURIComponent(searchQuery.trim())}`);
      setSearchQuery(""); // Clear search after navigation
    } else {
      // If empty, just go to gallery
      navigate("/gallery");
    }
  };

  return (
    <header className="sticky top-0 z-50 w-full border-b bg-gradient-to-r from-white via-primary/8 to-secondary/8 backdrop-blur supports-[backdrop-filter]:bg-white/95 shadow-sm">
